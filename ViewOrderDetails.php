...
{
	$order_id=$_GET['id'];
	$result1=mysqli_query($connect,"SELECT customer_order.product_id,product.Product_Name,product.Product_Price,customer_order.quantity FROM(customer_order 
                                    INNER JOIN product ON customer_order.product_id = product.Product_ID) WHERE payment_id='$order_id'");	
